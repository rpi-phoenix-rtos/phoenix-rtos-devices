...
/* Return type is ssize_t to match cache_readCb_t (libcache); on aarch64 ssize_t
 * is 64-bit, so the original 'int' tripped -Wincompatible-pointer-types. */
static ssize_t sdcard_readCb(uint64_t offs, void *buff, size_t len, cache_devCtx_t *ctx)
{
...
static ssize_t sdcard_writeCb(uint64_t offs, const void *buff, size_t len, cache_devCtx_t *ctx)
{
	if (ctx->noFlushShutdown) {
		return len;
	}
