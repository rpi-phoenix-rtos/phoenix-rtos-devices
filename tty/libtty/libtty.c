...
int _libtty_ioctl(libtty_common_t *tty, pid_t sender_pid, unsigned long cmd, const void *in_arg, void *out_arg)
{
	...
	switch (cmd) {
		...
		case FIONREAD:
			 ...
			 * Rewritten for the new ioctl contract: the caller provides the out buffer
			 * and tty->temp is gone, so copy into out_arg the way TIOCGHALFD does. */
			val = (int)fifo_count(tty->rx_fifo);
			memcpy(out_arg, &val, sizeof(val));
			log_ioctl("FIONREAD = %d", val);
			break;
